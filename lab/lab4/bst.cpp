/*
	...
	CPSC 2121-002
	September 17, 2015
	Lab 4 - Binary Search Tree Operations
*/
...
Node *join(Node *L, Node *R)
{
	if (L == NULL) return R;
	if (R == NULL) return L;
	int rndm = (rand() % (L->size + R->size));

	if (rndm < L->size) { // L's root chosen as new root
		L->right = join (L->right, R);
		fix_size(L);
		return L;
	}		
	else {	// R's root chosen as new root
